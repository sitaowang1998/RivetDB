...
async fn node_recovers_committed_state() {
    for backend in all_backends() {
        ...
        let config = RivetConfig::new(42, "127.0.0.1:7800", Vec::new(), data_dir.clone())
            .with_storage(storage.storage_config());

        let node = RivetNode::new(config.clone(), storage.storage())
            .await
        ...
        let recovered_node = RivetNode::new(config, storage.reopen())
            .await
            .expect("recover node");
    ...
    let mut reader = recovered_manager.begin_transaction();
        let recovered = recovered_manager
            ...
            .expect("read result");
        match backend {
            common::BackendKind::Disk => {
                let value = recovered.expect("value present");
                assert_eq!(value.value, b"value");
            }
            common::BackendKind::Memory => {
                assert!(recovered.is_none(), "memory backend should not persist");
            }
        }
        ...
        if backend == common::BackendKind::Disk {
            assert!(
